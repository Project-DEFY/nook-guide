export default async (req, context) => {
  ...
  if (!notionKey) {
    return new Response(JSON.stringify({ error: 'NOTION_API_KEY not configured' }), { status: 500, headers });
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ html, title }), { status: 200, headers });
  } catch (err) {
